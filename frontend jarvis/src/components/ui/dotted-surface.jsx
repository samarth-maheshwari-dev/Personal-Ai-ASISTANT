...
export function DottedSurface({ className, ...props }) {
	// Simple mock for useTheme to stay dark-first as requested
	const theme = 'dark';
	...
	useEffect(() => {
		...
		scene.fog = new THREE.Fog(0x05070a, 2000, 10000);
		...
		for (let ix = 0; ix < AMOUNTX; ix++) {
			for (let iy = 0; iy < AMOUNTY; iy++) {
				...
				if (theme === 'dark') {
					colors.push(0.3, 0.4, 1.0); // Subtle blue tint for Jarvis
				} else {
					colors.push(0, 0, 0);
				}
			}
